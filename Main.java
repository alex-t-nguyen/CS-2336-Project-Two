...
public class Main {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException
	{
        ...
        LinkList<Player> list = new LinkList<>(); // Create linked list of players
       ...
       // Print league header
        ...
        for(int i = 0; i <= 5; i++)
        {
            ...
            switch(i)
            {
                ...
                */
                case 0: // Case for displaying batting average leaders
                    {
                        ...
                        sortDescendingBattingAverage(head); // Sort list in descending order
                LinkList<Player> firstLeaders = new LinkList<>();
                // ArrayList<String> firstLeaders = new ArrayList<>(); // Create arraylist for
                // first leaders names
                bWriter.write("\nBATTING AVERAGE\n"); // Display leader stat header
                int numAtBats = head.getData().calculateNumAtBats(head.getData().getHits(), head.getData().getOuts(),
                ...
                bWriter.write(String.format("%.3f",
                        head.getData().calculateBattingAverage(head.getData().getHits(), numAtBats)) + "\t");
                // bWriter.write(String.format("%.3f",
                // head.getData().calculateBattingAverage(head.getData().getHits(), numAtBats))
                ...
                while (iter != null) {
                    ...
                    iterNumAtBats = iter.getData().calculateNumAtBats(iter.getData().getHits(),
                            iter.getData().getOuts(), iter.getData().getStrikeouts());
                    iterBattingAverage = iter.getData().calculateBattingAverage(iter.getData().getHits(),
                            iterNumAtBats);
                    ...
                    if (iterBattingAverage == headBattingAverage) {
                        ...
                        // bWriter.write(", " + iter.getData().getName()); // <------ Append node to new
                        // ties linked list instead of printing name
                        firstCounter++;
                        tie = true;
                    }

                    // If not equal stop looping and set head to player where batting average was
                    // not equal (a.k.a 2nd place leader)
                    else {
                ...
                // list.sortArrayListNames(firstLeaders); // Sort array list of first leader
                // names
                for (Node<Player> j = firstLeaders.getHead(); j != null; j = j.getNext()) // Print names of first
                                                                                          // leaders
                {
                ...
                /*
                 * for(int j = 0; j < firstLeaders.size(); j++) // Display names of all first
                 * leaders { if(j == 0) bWriter.write(firstLeaders.get(j)); else
                ...
                if (totalNumLeaders <= 2) // Check if there are 3 or more leaders already
                {
                    ...
                    numAtBats = head.getData().calculateNumAtBats(head.getData().getHits(), head.getData().getOuts(),
                            head.getData().getStrikeouts());
                    headBattingAverage = head.getData().calculateBattingAverage(head.getData().getHits(), numAtBats);
                    ...
                    // head.getData().getName()); // <-------- Append Node to ties linked list
                    // instead of printing
                    ...
                    while (iter != null) {
                        ...
                        iterBattingAverage = iter.getData().calculateBattingAverage(iter.getData().getHits(),
                                iterNumAtBats);
                        headBattingAverage = head.getData().calculateBattingAverage(head.getData().getHits(),
                                numAtBats);
                        ...
                        // If not equal stop looping and set head to player where batting average was
                        // not equal (a.k.a 3rd place leader)
                        else {
                    ...
                    // list.sortArrayListNames(secondLeaders); // Sort second leaders array list
                    // alphabetically
                     ...
                     * { if(j == 0) bWriter.write(secondLeaders.get(j)); else bWriter.write(", " +
                     * secondLeaders.get(j)); }
                     */
                } else // If ties for 1st were >= 3 do not print out anything and end loop for that
                       // stat
                {
                    bWriter.write("\n");
                    break;
                ...
                if (totalNumLeaders <= 2) // Check if there are already 3 or more leaders already
                {
                    ...
                    // -----------> Deconstruct list to clear it of 1st place tied players by
                    // calling deconstruct function
                    numAtBats = head.getData().calculateNumAtBats(head.getData().getHits(), head.getData().getOuts(),
                            head.getData().getStrikeouts());
                    headBattingAverage = head.getData().calculateBattingAverage(head.getData().getHits(), numAtBats);
                    ...
                    bWriter.write("\n" + String.format("%.3f", headBattingAverage) + "\t");
                    // bWriter.write("\n" + String.format("%.3f", headBattingAverage) + "\t" +
                    // head.getData().getName()); // <------ Append node to new ties linked list
                    ...
                    while (iter != null) {
                        ...
                        // If not equal stop looping and print out nothing because there are no more
                        // leaders after 3rd
                        else {
                            // bWriter.write("\n");
                            break;
                    ...
                    // list.sortArrayListNames(thirdLeaders); // Sort third leaders array list
                    // alphabetically
                     ...
                     * if(j == 0) bWriter.write(thirdLeaders.get(j)); else bWriter.write(", " +
                     * thirdLeaders.get(j)); }
                     */
                    // bWriter.write("\n");
                }
            ...
            {
                sortDescendingOnBasePercentage(head); // Sort list of players descending
                LinkList<Player> firstLeaders = new LinkList<>();
                // ArrayList<String> firstLeaders = new ArrayList<>(); // arraylist for first
                // leaders
                bWriter.write("\nON-BASE PERCENTAGE\n"); // leader stat header
                int numPlateAppearances = head.getData().calculatePlateAppearances(head.getData().getHits(),
                        head.getData().getOuts(), head.getData().getStrikeouts(), head.getData().getWalks(),
                        head.getData().getHitByPitches(), head.getData().getSacrifices());
                ...
                bWriter.write(String
                        .format("%.3f", head.getData().calculateOnBasePercentage(head.getData().getHits(),
                                head.getData().getWalks(), head.getData().getHitByPitches(), numPlateAppearances))
                        + "\t");
                // bWriter.write(String.format("%.3f",
                // head.getData().calculateOnBasePercentage(head.getData().getHits(),
                // head.getData().getWalks(), head.getData().getHitByPitches(),
                // numPlateAppearances)) + "\t" + head.getData().getName());
                Node<Player> iter = head.getNext();
                ...
                while (iter != null) {
                    ...
                    iterNumPlateAppearances = iter.getData().calculatePlateAppearances(iter.getData().getHits(),
                            iter.getData().getOuts(), iter.getData().getStrikeouts(), iter.getData().getWalks(),
                            iter.getData().getHitByPitches(), iter.getData().getSacrifices());
                    iterOnBasePercentage = iter.getData().calculateOnBasePercentage(iter.getData().getHits(),
                            iter.getData().getWalks(), iter.getData().getHitByPitches(), iterNumPlateAppearances);
                    headOnBasePercentage = head.getData().calculateOnBasePercentage(head.getData().getHits(),
                            head.getData().getWalks(), head.getData().getHitByPitches(), numPlateAppearances);
                    ...
                    // If not equal stop looping and set head to player where on-place percentage
                    // was not equal (a.k.a 2nd place leader)
                    else {
                        head = iter; // Move head to 2nd place leader
                        iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                        break;
                 ...
                 * { if(j == 0) bWriter.write(firstLeaders.get(j)); else bWriter.write(", " +
                 * firstLeaders.get(j)); }
                 */
                totalNumLeaders += firstCounter; // Increase total number of leaders
                if (totalNumLeaders <= 2) // Check if there are 3 or more leaders already
                {
                    ...
                    LinkList<Player> secondLeaders = new LinkList<>();
                    // ArrayList<String> secondLeaders = new ArrayList<>();
                    numPlateAppearances = head.getData().calculatePlateAppearances(head.getData().getHits(),
                            head.getData().getOuts(), head.getData().getStrikeouts(), head.getData().getWalks(),
                            head.getData().getHitByPitches(), head.getData().getSacrifices());
                    headOnBasePercentage = head.getData().calculateOnBasePercentage(head.getData().getHits(),
                            head.getData().getWalks(), head.getData().getHitByPitches(), numPlateAppearances);
                    ...
                    while (iter != null) {
                        ...
                        // If not equal stop looping and set head to player where on-base percentage was
                        // not equal (a.k.a 3rd place leader)
                        else {
                            head = iter; // Move head to 3rd place leader
                            iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                    ...
                    totalNumLeaders += secondCounter; // Increase total number of leaders
                    // list.sortArrayListNames(secondLeaders); // Sort second leaders names
                    // alphabetically
                    secondLeaders.sortAlphabetically(secondLeaders.getHead());
                    ...
                    /*
                     * for(int j = 0; j < secondLeaders.size(); j++) // Print out second leaders
                     * names { if(j == 0) bWriter.write(secondLeaders.get(j)); else
                ...
                if (totalNumLeaders <= 2) // Check if there are already 3 or more leaders already
                {
                    ...
                    // list.sortArrayListNames(thirdLeaders); // Sort third leaders names
                    // alphabetically
                    thirdLeaders.sortAlphabetically(thirdLeaders.getHead());
            ...
            {
                sortDescendingHits(head); // Sort list of players
                ...
                while (iter != null) {
                    ...
                    // If not equal stop looping and set head to player where number of hits was not
                    // equal (a.k.a 2nd place leader)
                    else {
                        head = iter; // Move head to 2nd place leader
                        iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                        break;
                ...
                // list.sortArrayListNames(firstLeaders); // Sort first leaders names
                // alphabetically
                /*
            ...
            {
                sortDescendingWalks(head); // Sort list descending by walks
                LinkList<Player> firstLeaders = new LinkList<>();
                ...
                while (iter != null) {
                    ...
                    // If not equal stop looping and set head to player where number of walks was
                    // not equal (a.k.a 2nd place leader)
                    else {
                        head = iter; // Move head to 2nd place leader
                        iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                ...
                // list.sortArrayListNames(firstLeaders); // Sort first leaders arraylist
                // alphabetically
                /*
                 * for(int j = 0; j < firstLeaders.size(); j++) // Print first leaders names {
                 * if(j == 0) bWriter.write(firstLeaders.get(j)); else bWriter.write(", " +
                ...
                if (totalNumLeaders <= 2) // Check if there are 3 or more leaders already
                {
                    ...
                    // list.sortArrayListNames(secondLeaders); // Sort second eladers names
                    // alphabetically
                    /*
                     * for(int j = 0; j < secondLeaders.size(); j++) // Display second leader's
                     * names { if(j == 0) bWriter.write(secondLeaders.get(j)); else
                     * bWriter.write(", " + secondLeaders.get(j)); }
                     */
                } else // If ties for 1st were >= 3 do not print out anything and end loop for that
                ...
                if (totalNumLeaders <= 2) // Check if there are already 3 or more leaders already
                {
                    ...
                    /*
                     * for(int j = 0; j < thirdLeaders.size(); j++) // Print third leaders array
                     * list { if(j == 0) bWriter.write(thirdLeaders.get(j)); else bWriter.write(", "
                     * + thirdLeaders.get(j)); }
                     */
                    // bWriter.write("\n");
                }
            ...
            {
                sortAscendingStrikeouts(head); // Sort list ascending by strikeouts
                // Create first leaders array list
                LinkList<Player> firstLeaders = new LinkList<>();
                // ArrayList<String> firstLeaders = new ArrayList<>();
                ...
                while (iter != null) {
                    ...
                    // If not equal stop looping and set head to player where number of strikeouts
                    // was not equal (a.k.a 2nd place leader)
                    else {
                        head = iter; // Move head to 2nd place leader
                        iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                ...
                // list.sortArrayListNames(firstLeaders); // Sort first leaders array list
                // alphabetically
                /*
                 * for(int j = 0; j < firstLeaders.size(); j++) // Print first leaders names {
                 * if(j == 0) bWriter.write(firstLeaders.get(j)); else bWriter.write(", " +
                ...
                if (totalNumLeaders <= 2) // Check if there are 3 or more leaders already
                {
                    ...
                    while (iter != null) {
                        ...
                        // If not equal stop looping and set head to player where number of strikeouts
                        // was not equal (a.k.a 3rd place leader)
                        else {
                            head = iter; // Move head to 3rd place leader
                            iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                            break;
                    ...
                    /*
                     * for(int j = 0; j < secondLeaders.size(); j++) // Print second leaders { if(j
                     * == 0) bWriter.write(secondLeaders.get(j)); else bWriter.write(", " +
                     * secondLeaders.get(j)); }
                     */
                } else // If ties for 1st were >= 3 do not print out anything and end loop for that
                       // stat
            ...
            case 5: // case for displaying hit by pitch leaders
            {
                ...
                sortDescendingHitByPitches(head);
                ...
                while (iter != null) {
                    ...
                    // If not equal stop looping and set head to player where number of hit by
                    // pitches was not equal (a.k.a 2nd place leader)
                    else {
                        head = iter; // Move head to 2nd place leader
                        iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                        break;
                ...
                // list.sortArrayListNames(firstLeaders); // Sort first leaders list
                // alphabetically
                /*
                 * for(int j = 0; j < firstLeaders.size(); j++) // Print first leaders names {
                 * if(j == 0) bWriter.write(firstLeaders.get(j)); else bWriter.write(", " +
                ...
                if (totalNumLeaders <= 2) // Check if there are 3 or more leaders already
                {
                    ...
                    while (iter != null) {
                        ...
                        // If not equal stop looping and set head to player where number of hit by
                        // pitches was not equal (a.k.a 3rd place leader)
                        else {
                            head = iter; // Move head to 3rd place leader
                            iter = iter.getNext(); // Move iterator to next player after 2nd place leader
                            break;
     ...
     * @param h head of linked list to sort
     * @return head node of list
     */
    public static Node<Player> sortDescendingHits(Node<Player> h) {
        Node<Player> head;
        boolean swapped = true;
        if (h == null || h.getNext() == null) // If only 1 node in list or none
            return h;
        else {
            while (swapped) {
                swapped = false;
                ...
                while (iter != null) {
                    ...
                    if (head.getData().getHits() < iter.getData().getHits()) {
                        // Swap
                        Node<Player> temp = new Node<>();
     ...
     * @return head node of list
     */
    public static Node<Player> sortDescendingOuts(Node<Player> h) {
        Node<Player> head;
        boolean swapped = true;
        if (h == null || h.getNext() == null) // If only 1 node in list or none
            return h;
            ...
            while (swapped) {
                ...
                while (iter != null) {
                    ...
                    if (head.getData().getOuts() > iter.getData().getOuts()) {
                        // Swap
                        Node<Player> temp = new Node<>();
                        temp.setData(head.getData());
                        head.setData(iter.getData());
     ...
     * @return head node of list
     */
    public static Node<Player> sortAscendingStrikeouts(Node<Player> h) {
        Node<Player> head;
        boolean swapped = true;
        if (h == null || h.getNext() == null) // If only 1 node in list or none
            return h;
            ...
            while (swapped) {
                ...
                while (iter != null) {
                    ...
                    if (head.getData().getStrikeouts() > iter.getData().getStrikeouts()) {
                        // Swap
                        Node<Player> temp = new Node<>();
                        temp.setData(head.getData());
                        head.setData(iter.getData());
     ...
     * @return head node of list
     */
    public static Node<Player> sortDescendingWalks(Node<Player> h) {
        Node<Player> head;
        boolean swapped = true;
        if (h == null || h.getNext() == null) // If only 1 node in list or none
            return h;
            ...
            while (swapped) {
                ...
                while (iter != null) {
                    ...
                    if (head.getData().getWalks() < iter.getData().getWalks()) {
                        // Swap
                        Node<Player> temp = new Node<>();
                        temp.setData(head.getData());
                        head.setData(iter.getData());
     ...
     * @return head node of list
     */
    public static Node<Player> sortDescendingHitByPitches(Node<Player> h) {
        Node<Player> head;
        boolean swapped = true;
        if (h == null || h.getNext() == null) // If only 1 node in list or none
            return h;
            ...
            while (swapped) {
                ...
                while (iter != null) {
                    ...
                    if (head.getData().getHitByPitches() < iter.getData().getHitByPitches()) {
                        // Swap
                        Node<Player> temp = new Node<>();
                        temp.setData(head.getData());
                        head.setData(iter.getData());
     ...
     * @return head node of list
     */
    public static Node<Player> sortDescendingSacrifices(Node<Player> h) {
        Node<Player> head;
        boolean swapped = true;
        if (h == null || h.getNext() == null) // If only 1 node in list or none
            return h;
            ...
            while (swapped) {
                ...
                while (iter != null) {
                    ...
                    if (head.getData().getSacrifices() < iter.getData().getSacrifices()) {
                        // Swap
                        Node<Player> temp = new Node<>();
                        temp.setData(head.getData());
                        head.setData(iter.getData());
     ...
     * @return head node of list
     */
    public static Node<Player> sortDescendingBattingAverage(Node<Player> h) {
        Node<Player> head;
        boolean swapped = true;
        if (h == null || h.getNext() == null) // If only 1 node in list or none
            return h;
            ...
            while (swapped) {
                ...
                while (iter != null) {
                    ...
                    headAtBats = head.getData().calculateNumAtBats(head.getData().getHits(), head.getData().getOuts(),
                            head.getData().getStrikeouts());
                    iterAtBats = iter.getData().calculateNumAtBats(iter.getData().getHits(), iter.getData().getOuts(),
                            iter.getData().getStrikeouts());
                    headBattingAverage = head.getData().calculateBattingAverage(head.getData().getHits(), headAtBats);
                    iterBattingAverage = iter.getData().calculateBattingAverage(iter.getData().getHits(), iterAtBats);
     ...
     * @return head node of list
     */
    public static Node<Player> sortDescendingOnBasePercentage(Node<Player> h)
    {
        Node<Player> head;
        boolean swapped = true;
        if(h == null || h.getNext() == null) // If only 1 node in list or none
        ...
        else
        {
            ...
            while(swapped)
            {
                ...
                while(iter != null)
                {
                    ...
                    headPlateAppearances = head.getData().calculatePlateAppearances(head.getData().getHits(), head.getData().getOuts(), head.getData().getStrikeouts(), head.getData().getWalks(), head.getData().getHitByPitches(), head.getData().getSacrifices());         
                    iterPlateAppearances = iter.getData().calculatePlateAppearances(iter.getData().getHits(), iter.getData().getOuts(), iter.getData().getStrikeouts(), iter.getData().getWalks(), iter.getData().getHitByPitches(), iter.getData().getSacrifices());
                    headOnBasePercentage = head.getData().calculateOnBasePercentage(head.getData().getHits(), head.getData().getWalks(), head.getData().getHitByPitches(), headPlateAppearances);
                    iterOnBasePercentage = iter.getData().calculateOnBasePercentage(iter.getData().getHits(), iter.getData().getWalks(), iter.getData().getHitByPitches(), iterPlateAppearances);
